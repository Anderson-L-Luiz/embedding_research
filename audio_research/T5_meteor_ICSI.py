...
# Tokenize the text for METEOR scoring
def tokenize(text):
    return text.split()
...
# Initialize list to store METEOR scores and summaries
meteor_scores = []
...
for chunk_file, llama_file in zip(chunk_files, llama_summary_files):
    ...
    # Tokenize both summaries for METEOR score calculation
    t5_summary_tokens = tokenize(t5_summary)
    llama_summary_tokens = tokenize(llama_summary)
    
    # Compute METEOR score comparing the T5 summary and the LLaMA summary
    score = meteor_score([llama_summary_tokens], t5_summary_tokens)
    meteor_scores.append(score)  # Store the METEOR score
    
    print(f"METEOR score for {chunk_file} (T5 vs LLaMA): {score}")

# Calculate the mean METEOR score
mean_meteor = np.mean(meteor_scores)

print(f"\nMean METEOR score across all chunks: {mean_meteor}")
