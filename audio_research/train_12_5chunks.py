...
# Step 4: Summarize the Transcribed Text with LLaMA 3 8B
...
# Summarization function with a concise prompt
def summarize_text_llama(text, model, tokenizer, max_new_tokens=2000):
    prompt = f"Summarize the following transcript into a brief and coherent summary:\n{text}"
    
    print(f"--- INPUT to LLaMA ---\n{prompt}\n")  # Print the input being sent to the model
    ...
    # Get the actual generated summary (the model's output)
    summary = tokenizer.decode(outputs[0], skip_special_tokens=True)
    
    print(f"--- OUTPUT from LLaMA ---\n{summary}\n")  # Print the model's generated summary
...
for chunk in all_text_chunks:
    summary = summarize_text_llama(chunk, llama_model, tokenizer)
