...
experiment_folder = "/home/anderson/audio_embedding_research/experiments/experiment_59"
...
def process_significance_and_summaries(experiment_folder):
    ...
    # Step 7: Apply ROUGE metric for text summaries
    ...
    # Save ROUGE scores for text to JSON file
    with open(os.path.join(experiment_folder, "rouge_text_scores.json"), "w") as f:
        json.dump(rouge_text_scores, f, indent=2)

    # Note: Since we cannot apply ROUGE to audio summaries, ROUGE scores for audio are not applicable.
    # However, you can implement other audio evaluation metrics if needed.
